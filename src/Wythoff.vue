...
const triangle = [2, 3, 5 / 2];

const vertices = wythoff.makeSphericalTriangle(
  Math.PI / triangle[0], Math.PI / triangle[1], Math.PI / triangle[2]
);
const points = wythoff.wythoff(triangle[0], triangle[1], triangle[2]);
...
onMounted(() => {
  ...
  let mesh: THREE.Mesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({ color: "#abcdef" }));
  scene.add(mesh);
