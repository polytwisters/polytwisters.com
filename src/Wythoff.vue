...
const symbolFromPolytwister = computed(() =>
  PolytwisterSymbol.from(props.symbol),
);
...
function makeMirrorDisk(normal: Vector3): THREE.BufferGeometry {
  ...
  for (let j = 0; j < numPoints; j++) {
    ...
    points.push(point.normalize().multiplyScalar(diskRadius));
  }
...
<template>
  <p class="flex flex-row gap-3">
    ...
    <input
      type="text"
      v-model="userSymbolText"
      :disabled="lockToPolytwister"
      :class="{ error: symbolError }"
    />
    <input
      type="checkbox"
      v-model="lockToPolytwister"
      id="lock-to-polytwister"
    />
