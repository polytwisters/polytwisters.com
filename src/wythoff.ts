...
function findVertex(vertices: Vertex[], position: Vector3): number {
  const result = _.findIndex(vertices, (vertex) => vertex.position.distanceTo(position) <= EPSILON);
  if (result === -1) {
...
function findEdge(edges: Edge[], position: Vector3): number {
  const result = _.findIndex(edges, (edge) => edge.position.distanceTo(position) <= EPSILON);
  if (result === -1) {
...
export class SchwarzTriangle {
  ...
  fundamentalMobiusTriangle(): Vector3[] {
    ...
    for (let i = 0; i < allMirrors.length; i++) {
      ...
      if (
        angle12 > EPSILON && angle13 > EPSILON
...
function dedupePoints(points: Vector3[]): Vector3[] {
  return _.uniqWith(points, (point1, point2) => point1.distanceTo(point2) <= EPSILON);
}
...
export class SymmetryGroup {
  ...
  orbitWithMatrices(point: Vector3): PointAndMatrix[] {
    ...
    result = _.uniqWith(result, (x1, x2) =>
      x1.position.distanceTo(x2.position) <= EPSILON
    );
    return result;
  }
  ...
  makePolyhedron(quasiregular: boolean): Polyhedron {
    ...
    let edges: Edge[] = this.orbitWithMatrices(baseEdgePosition).map((pm: PointAndMatrix) => {
      const vertex1Position = baseEdgeEndpoint1.clone().applyMatrix3(pm.matrix);
      const vertex2Position = baseEdgeEndpoint2.clone().applyMatrix3(pm.matrix);
    ...
    function makeFaces(center: Vector3, cornerRotation: Matrix3, symbol: Fraction, faceType: number): Face[] {
      ...
      return that.orbitWithMatrices(center).map((pm: PointAndMatrix) => {
        const vertexPositions = baseVertexPositions.map((p) => p.clone().applyMatrix3(pm.matrix));
        const edgePositions = baseEdgePositions.map((p) => p.clone().applyMatrix3(pm.matrix));
        return {
