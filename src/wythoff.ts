import { Matrix3, Vector3 } from "three";
import * as _ from "lodash";
import * as mathUtils from "./mathUtils";
import { type Fraction, type FractionLike, asFraction, fractionToString } from "./fraction";
import { type PolytwisterSymbolLike, PolytwisterSymbol } from "./symbol";

export class Polyhedron {
  vertexLocations: Vector3[];
  edges: Edge[];
  faces: Face[];

  constructor(vertexLocations: Vector3[], edges: Edge[], faces: Face[]) {
    this.vertexLocations = vertexLocations;
    this.edges = edges;
    this.faces = faces;
  }

  getAdjacentFaceIndices(faceIndex: number): number[] {
    const face = this.faces[faceIndex];
    const result: number[] = [];
    for (const [faceIndex2, face2] of this.faces.entries()) {
      if (faceIndex2 === faceIndex) {
        continue;
      }
      if (facesAdjacent(face, face2)) {
        result.push(faceIndex2);
      }
    }
    return result;
  }
}

interface Face {
  // These are the indices of vertices, not their locations. (vertexIndices felt too long.)
  vertices: number[],
  // These are the indices of edges, not their locations. (edgeIndices felt too long.)
  edges: number[],
  center: Vector3
}

interface Edge {
  vertex1: number,
  vertex2: number,
  // The "location" of the edge doesn't really have much geometrical meaning, but it must respect
  // the symmetry of the polyhedron generated by the Wythoffian construction. It helps distinguish
  // different edges with the same endpoints, e.g. in the dysters and sheaved polytwisters.
  location: Vector3,
}

interface Vertex {
  position: Vector3,
  matrix: Matrix3
}

function schwarzAngle(fraction: Fraction): number {
  return Math.PI / (fraction.n / fraction.d);
}

function isNearInteger(n: number): boolean {
  return Math.abs(n - Math.round(n)) <= 1e-3;
}

const GENERATOR_POINT = new Vector3(0, 0, 1);

/**
 * A Schwarz triangle is a spherical triangle that tiles the sphere. It is specified as (a b c)
 * where the three interior angles are pi/a pi/b pi/c.
 */
export class SchwarzTriangle {
  n1: Fraction;
  n2: Fraction;
  n3: Fraction;

  constructor(n1: FractionLike, n2: FractionLike, n3: FractionLike) {
    this.n1 = asFraction(n1);
    this.n2 = asFraction(n2);
    this.n3 = asFraction(n3);
    this.check();
  }

  get angle1() { return schwarzAngle(this.n1); }
  get angle2() { return schwarzAngle(this.n2); }
  get angle3() { return schwarzAngle(this.n3); }

  static fromSymbol(symbol: PolytwisterSymbolLike) {
    const symbol2 = PolytwisterSymbol.from(symbol);
    return new SchwarzTriangle(symbol2.ring, symbol2.twister1, symbol2.twister2);
  }

  asString() {
    return `(${fractionToString(this.n1)} ${fractionToString(this.n2)} ${fractionToString(this.n3)})`;
  }

  private check() {
    if (this.angle1 + this.angle2 + this.angle3 <= Math.PI) {
      throw new Error(`Invalid spherical triangle ${this.asString()}: angles must sum to more than 180 degrees`);
    }
  }

  /**
   * Given three interior angles of a spherical triangle, return the side length of the side opposite
   * the FIRST angle.
   */
  static sphericalTriangleSide(angle1: number, angle2: number, angle3: number): number {
    return Math.acos(
      (Math.cos(angle1) + Math.cos(angle2) * Math.cos(angle3)) /
      (Math.sin(angle2) * Math.sin(angle3))
    );
  }

  /**
   * Given three interior angles of a spherical triangle, produce three unit vectors realizing that
   * spherical triangle on the unit sphere. The first unit vector is guaranteed to be (0, 0, 1),
   * and the second is guaranteed to be of the form (x, 0, z).
   */
  vertices(): Vector3[] {
    const point1 = new Vector3(0, 0, 1);
    const side12 = SchwarzTriangle.sphericalTriangleSide(this.angle3, this.angle1, this.angle2);
    const side13 = SchwarzTriangle.sphericalTriangleSide(this.angle2, this.angle1, this.angle3);
    const point2 = mathUtils.fromSpherical(1, 0, Math.PI / 2 - side12);
    const point3 = mathUtils.fromSpherical(1, this.angle1, Math.PI / 2 - side13);
    const result = [point1, point2, point3];
    return result;
  }

  /**
   * Produce a set of three vectors which are the normals of the three planes that cut the triangle.
   */
  mirrors(): Vector3[] {
    const points = this.vertices();
    return [
      points[1].clone().cross(points[2]).normalize(),
      points[2].clone().cross(points[0]).normalize(),
      points[0].clone().cross(points[1]).normalize()
    ];
  }

  static mobiusFromPoints(p2: Vector3, p3: Vector3): SchwarzTriangle {
    if (Math.abs(p2.y) > 1e-3) {
      throw new Error("Y-coordinate of p2 must be 0");
    }
    const p1 = GENERATOR_POINT;
    const angle1 = mathUtils.reject(p2, p1).angleTo(mathUtils.reject(p3, p1));
    const angle2 = mathUtils.reject(p3, p2).angleTo(mathUtils.reject(p1, p2));
    const angle3 = mathUtils.reject(p1, p3).angleTo(mathUtils.reject(p2, p3));
    const n1 = Math.round(Math.PI / angle1);
    const n2 = Math.round(Math.PI / angle2);
    const n3 = Math.round(Math.PI / angle3);
    return new SchwarzTriangle(n1, n2, n3);
  }

  /**
   * Find a Mobius triangle which is a fundamental triangle of this one. 
   */
  fundamentalMobiusTriangle(): Vector3[] {
    const mirrors = this.mirrors();
    const group = SymmetryGroup.fromSchwarzTriangle(this);
    let allMirrors = [
      ...group.orbit(mirrors[0]),
      ...group.orbit(mirrors[1]),
      ...group.orbit(mirrors[2]),
    ];
    allMirrors = dedupeMirrors(allMirrors);

    // Pick one mirror.
    let mirror1 = allMirrors[0];

    // Find a mirror that has the smallest nonzero angle with the first mirror. The possible Mobius
    // triangles are (p 2 2) (3 3 2) (4 3 2) (5 3 2). This angle corresponds to the first number in
    // the Mobius triangle.
    let leastAngle = Infinity;
    let mirror2 = null;
    for (let i = 0; i < allMirrors.length; i++) {
      const candidate = allMirrors[i];
      const angle = candidate.angleTo(mirror1);
      if (angle > 1e-3 && angle < leastAngle) {
        mirror2 = candidate;
        leastAngle = angle;
      }
    }
    if (mirror2 === null) {
      throw new Error("Shouldn't happen");
    }
  
    let mirror3 = null;
    for (let i = 0; i < allMirrors.length; i++) {
      const candidate = allMirrors[i];
      const angle13 = candidate.angleTo(mirror1);
      const angle12 = candidate.angleTo(mirror2);
      if (
        angle12 > 1e-3 && angle13 > 1e-3
        && isNearInteger(Math.PI / angle13)
        && isNearInteger(Math.PI / angle12)
        && (
          Math.round(Math.PI / angle12) === 2
          || Math.round(Math.PI / angle13) === 2
        )
      ) {
        return [mirror1, mirror2, candidate];
      }
    }

    throw new Error("Shouldn't happen");
  }
}

/**
 * Given a Coxeter word as an array of integers and an array of 3x3 matrices, multiply the matrices
 * together to produce the matrix corresponding to that word. For example, if the Coxeter word is
 * [2, 1, 0, 2] and the matrices are [M0, M1, M2], then the result will be the matrix multiplication
 * M2 M1 M0 M2.
 */
function realizeCoxeterWord(word: number[], operators: Matrix3[]): Matrix3 {
  let result = new Matrix3();
  for (let operator of word) {
    result = result.multiply(operators[operator]);
  }
  return result;
}

interface Symmetry {
  matrix: Matrix3,
  coxeterWord: number[],
}

function dedupePoints(points: Vector3[]): Vector3[] {
  const result: Vector3[] = [];
  for (let point of points) {
    if (!result.some((point2) => point.distanceTo(point2) <= 1e-3)) {
      result.push(point);
    }
  }
  return result;
}

function dedupeMirrors(mirrors: Vector3[]): Vector3[] {
  const result: Vector3[] = [];
  for (let mirror of mirrors) {
    if (!result.some((mirror2) => mirror.dot(mirror2) >= 1 - 1e-3)) {
      result.push(mirror);
    }
  }
  return result;
}

/**
 * A symmetry group, or more accurately a representation of a finite subgroup of O(3). The group is
 * encoded as a set of 3x3 orthogonal matrices ("symmetries") with the following properties:
 * 
 * 1. If matrices A and B are symmetries, AB is also a symmetry.
 * 2. If A is a symmetry, A^-1 is also a symmetry.
 * 
 * In this software point groups are produced algorithmically and generated from a set of three
 * mirrors, and all point groups are achiral.
 */
export class SymmetryGroup {
  // The operators are three reflection matrices that generate the group. The first two operators
  // are guaranteed to leave GENERATOR_POINT (0, 0, 1) invariant, while the third one is not.
  operators: Matrix3[];
  symmetries: Symmetry[];
  schwarzTriangle: SchwarzTriangle;

  constructor(schwarzTriangle: SchwarzTriangle, operators: Matrix3[], symmetries: Symmetry[]) {
    this.operators = operators;
    this.symmetries = symmetries;
    this.schwarzTriangle = schwarzTriangle;
  }

  get order(): number {
    return this.symmetries.length;
  }

  /**
   * Given a description of a Schwarz triangle, produce the symmetry group generated by placing
   * three mirrors at its edges.
   */
  static fromSchwarzTriangle(schwarzTriangle: SchwarzTriangle): SymmetryGroup {
    // Gather three reflection operators which generate the group, notated R1 R2 R3 in later
    // comments.
    const mirrors = schwarzTriangle.mirrors();
    const operators: Matrix3[] = mirrors.map((mirror) =>
      mathUtils.householder(mirror)
    );
    // Stack for depth-first search is initialized with the identity operator.
    const stack: Symmetry[] = [
      { matrix: new Matrix3(), coxeterWord: [] }
    ];
    const elements: Symmetry[] = [];

    while (true) {
      const candidate = stack.pop();
      // If the stack is empty, exit.
      if (!candidate) {
        break;
      }
      // If any existing element in the point group has a matrix which is identical (with a
      // floating-point fudge factor) then ignore it.
      if (elements.some((x) => mathUtils.matrixMaxError(x.matrix, candidate.matrix) < 1e-3)) {
        continue;
      }
      // This is now officially a new element E of the point group.
      elements.push(candidate);

      if (elements.length > 120) {
        throw new Error("Maximum group order exceeded. Infinite Coxeter group?");
      }

      // For the three operators, add R1*E, R2*E, R3*E to the stack. Record their matrices and their
      // Coxeter words.
      for (let operator = 0; operator < 3; operator++) {
        const coxeterWord = [operator].concat(candidate.coxeterWord);
        const matrix = realizeCoxeterWord(coxeterWord, operators);
        stack.push({ matrix, coxeterWord });
      }
    }

    return new SymmetryGroup(schwarzTriangle, operators, elements);
  }

  orbit(point: Vector3): Vector3[] {
    let result = this.symmetries.map((element) =>
      point.clone().applyMatrix3(element.matrix)
    );
    result = dedupePoints(result);
    return result;
  }

  chiralOrbit(point: Vector3): Vector3[] {
    let result = this.symmetries.filter((x) =>
      x.coxeterWord.length % 2 === 0
    ).map((element) =>
      point.clone().applyMatrix3(element.matrix)
    );
    result = dedupePoints(result);
    return result;
  }

  makePolyhedron(quasiregular: boolean): Polyhedron {
    const schwarzTriangleVertices = this.schwarzTriangle.vertices();

    // Apply all elements in the group to the vertex, keeping track of the matrices used to produce
    // them.
    let vertexCandidates = this.symmetries.map((element) =>
      ({
        position: GENERATOR_POINT.clone().applyMatrix3(element.matrix),
        matrix: element.matrix
      })
    );

    // Deduplicate the vertices.
    const vertices: Vertex[] = [];
    for (let vertex of vertexCandidates) {
      if (!vertices.some((vertex2) => vertex.position.distanceTo(vertex2.position) <= 1e-3)) {
        vertices.push(vertex);
      }
    }

    let edges: Edge[] = [];

    // Produce edges by computing the orbit of a single edge.
    // 1. If p is the generator point, then R1*p is an adjacent vertex. (R2 p = R3 p = p, so there's
    // no need to check those; consequently the polyhedron is edge-transitive.) So
    //
    //     [p, R1*p]
    //
    // gives us an edge.
    //
    // 2. All other edges are formed by taking all elements of the group and transforming that edge.
    for (let element of this.symmetries) {
      const baseEdgeEndpoint1 = GENERATOR_POINT.clone();
      const baseEdgeEndpoint2 = GENERATOR_POINT.clone().applyMatrix3(this.operators[0]);
      // See the comment on the Edge interface -- the location is just to identify different edges
      // in e.g. the dysters & sheaved polytwisters, has no real geometric meaning.
      const baseEdgeLocation = (
        schwarzTriangleVertices[1].clone().lerp(schwarzTriangleVertices[2], 0.5)
      );
      const vertex1Location = baseEdgeEndpoint1.applyMatrix3(element.matrix);
      const vertex2Location = baseEdgeEndpoint2.applyMatrix3(element.matrix);
      const location = baseEdgeLocation.applyMatrix3(element.matrix);
      const vertex1 = findVertex(vertex1Location, vertices);
      const vertex2 = findVertex(vertex2Location, vertices);
      if (vertex1 === null || vertex2 === null) {
        throw new Error("Can't find edge ending");
      }
      edges.push({ vertex1, vertex2, location });
    }
    edges = dedupeEdges(edges);

    /**
     * Given an "edge walker" matrix and a number of edges, produce a face object.
     * An edge walker is a matrix M that, given any vertex v, Mv is an adjacent
     * vertex. Thus v, Mv, M^2 v, M^3 v, etc. produces all vertices of the face.
     */
    function makeFace(edgeWalker: Matrix3, count: number, center: Vector3): Face {
      let vertex = GENERATOR_POINT;
      let face = [];
      for (let i = 0; i < count; i++) {
        vertex = vertex.clone().applyMatrix3(edgeWalker);
        const vertexIndex = findVertex(vertex, vertices);
        if (vertexIndex === null) {
          throw new Error("Vertex not found");
        }
        face.push(vertexIndex);
      }
      return {
        vertices: face,
        edges: [],
        center
      };
    }
  
    /**
     * Given a Face, produce all possible transformations of it using the point
     * group. The results are not deduplicated.
     */
    function symmetrizeFace(face: Face, elements: Matrix3[]): Face[] {
      const vertexLocations = face.vertices.map((x) => vertices[x].position);
      const result = [];
      for (let element of elements) {
        const vertexLocations2 = vertexLocations.map((x) => x.clone().applyMatrix3(element));
        const faceVertices = vertexLocations2.map((x) => {
          let result = findVertex(x, vertices);
          if (result === null) {
            throw new Error("Vertex not found");
          }
          return result;
        });
        const center = face.center.clone().applyMatrix3(element);
        result.push({ vertices: faceVertices, center, edges: [] });
      }
      return result;
    }

    const elementMatrices = this.symmetries.map((x) => x.matrix);

    let faces: Face[] = [];
    let face1 = makeFace(
      realizeCoxeterWord([0, 1], this.operators),
      this.schwarzTriangle.n3.n,
      schwarzTriangleVertices[2],
    );
    faces = faces.concat(symmetrizeFace(face1, elementMatrices));
    if (quasiregular) {
      let face2 = makeFace(
        realizeCoxeterWord([0, 2], this.operators),
        this.schwarzTriangle.n2.n,
        schwarzTriangleVertices[1],
      );
      faces = faces.concat(symmetrizeFace(face2, elementMatrices));
    }
    faces = dedupeFaces(faces);

    return new Polyhedron(
      vertices.map((x) => x.position),
      edges,
      faces
    );
  }
}

function findVertex(point: Vector3, vertices: Vertex[]): number | null {
  for (let [i, candidate] of vertices.entries()) {
    if (candidate.position.distanceTo(point) <= 1e-3) {
      return i;
    }
  }
  return null;
}

function dedupeEdges(edges: Edge[]): Edge[] {
  const result: Edge[] = [];
  for (let edge of edges) {
    let found = false;
    for (let edge2 of result) {
      if (edge.location.distanceTo(edge2.location) > 1e-3) {
        continue;
      }
      if (edge.vertex1 === edge2.vertex1 && edge.vertex2 === edge2.vertex2) {
        found = true;
        break;
      }
      if (edge.vertex1 === edge2.vertex2 && edge.vertex2 === edge2.vertex1) {
        found = true;
        break;
      }
    }
    if (found === false) {
      result.push(edge);
    }
  }
  return result;
}

function facesEqual(face1: Face, face2: Face): boolean {
  if (face1.center.distanceTo(face2.center) > 1e-3) {
    return false;
  }
  const vertices1 = face1.vertices;
  const vertices2 = face2.vertices;
  if (vertices1.length !== vertices2.length) {
    return false;
  }
  const n = vertices1.length;
  for (let offset = 0; offset < n; offset++) {
    let match = true;
    for (let i = 0; i < n; i++) {
      if (vertices1[i] !== vertices2[(offset + i) % n]) {
        match = false;
        break;
      }
    }
    if (match) {
      return true;
    }
    match = true;
    for (let i = 0; i < n; i++) {
      if (vertices1[i] !== vertices2[(offset - i + n) % n]) {
        match = false;
        break;
      }
    }
    if (match) {
      return true;
    }
  }
  return false;
}

function dedupeFaces(faces: Face[]): Face[] {
  const result: Face[] = [];
  for (let face of faces) {
    if (!result.some((candidate) => facesEqual(face, candidate))) {
      result.push(face);
    }
  }
  return result;
}

export function symbolToPolyhedron(symbol: PolytwisterSymbol): Polyhedron {
  return SymmetryGroup.fromSchwarzTriangle(
    SchwarzTriangle.fromSymbol(symbol)
  ).makePolyhedron(symbol.quasiregular);
}