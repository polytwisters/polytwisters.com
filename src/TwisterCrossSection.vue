...
const verticesOuter = computed(
  () => polytwister.value.outerRings[face.value.orbit],
);
const d = computed(() =>
...
const q = computed(() => arcPolygon.safeFloor(radiusIndex.value));
...
<template>
  <div>
    n = {{ numCircles }}, q = {{ q === Infinity ? "\u221e" : q }}, d = {{ d }},
    v = {{ verticesOuter ? "+" : "-" }}, i = {{ bloated ? "+" : "-" }}
    <canvas
      ref="canvas"
      :width="canvasWidth"
      :height="canvasHeight"
